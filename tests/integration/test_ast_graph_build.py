...
def test_cli_entrypoint_runs(tmp_path: Path, corpus_root: Path) -> None:
    ...
    script = Path(__file__).resolve().parent.parent.parent / "src" / "java_codebase_rag" / "graph" / "build_ast_graph.py"
    proc = subprocess.run(
        [
            sys.executable,
            str(script),
...
def _load_baseline() -> dict:
    ...
    baseline_path = Path(__file__).resolve().parent.parent / "fixtures" / "graph_baseline_bank_chat.json"
