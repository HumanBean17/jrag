...
_STUB_ROOT = Path(__file__).resolve().parent.parent / "fixtures" / "brownfield_client_stubs"
...
def _build_repeatable_clients(tmp_path: Path) -> Path:
    ...
    build_ladybug_full_into(tmp_path, db_path)
    return db_path


def test_http_calls_table_built_on_bank_chat(ladybug_db_path: Path) -> None:
    assert _scalar(ladybug_db_path, "MATCH (:Client)-[r:HTTP_CALLS]->(:Route) RETURN count(r)") >= 2


def test_async_calls_table_built_on_bank_chat(ladybug_db_path: Path) -> None:
    assert _scalar(ladybug_db_path, "MATCH (:Producer)-[r:ASYNC_CALLS]->(:Route) RETURN count(r)") >= 5


def test_pr_d1_emits_unresolved_match_for_all(ladybug_db_path: Path) -> None:
    assert _scalar(ladybug_db_path, "MATCH ()-[r:HTTP_CALLS]->() WHERE r.match <> 'unresolved' RETURN count(r)") == 0
    assert _scalar(ladybug_db_path, "MATCH ()-[r:ASYNC_CALLS]->() WHERE r.match <> 'unresolved' RETURN count(r)") == 0


def test_phantom_routes_dedup_across_call_sites(ladybug_db_path_http_caller_smoke: Path) -> None:
    db = ladybug_db_path_http_caller_smoke
    route_ids = _scalar(
        db,
...
def test_graph_meta_call_edge_counters(ladybug_db_path: Path) -> None:
    m = LadybugGraph(str(ladybug_db_path)).meta()
...
def test_ontology_version_matches_graph_meta(ladybug_db_path: Path) -> None:
    assert LadybugGraph(str(ladybug_db_path)).meta()["ontology_version"] == ONTOLOGY_VERSION


def test_call_edges_client_outbound_http_calls_returns_routes(ladybug_db_path_http_caller_smoke: Path) -> None:
    db = ladybug_db_path_http_caller_smoke
    n = _scalar(
        db,
...
def test_call_edges_cross_service_http_four_hop(ladybug_db_path_cross_service_smoke: Path) -> None:
    db = ladybug_db_path_cross_service_smoke
    n = _scalar(
        db,
...
def test_call_edges_cross_service_async_four_hop(ladybug_db_path_cross_service_smoke: Path) -> None:
    db = ladybug_db_path_cross_service_smoke
    n = _scalar(
        db,
