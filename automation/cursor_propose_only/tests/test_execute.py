...
from automation.cursor_propose_only.execute import CommandResult, execute_workflow, parse_cursor_prompts
...
def test_parse_cursor_prompts_extracts_ordered_tasks() -> None:
    tasks = parse_cursor_prompts(_PROMPTS_FIXTURE)
...
def test_parse_cursor_prompts_requires_prompt_block() -> None:
    with pytest.raises(ValueError):
        parse_cursor_prompts("## PR-A1 — Missing prompt\n**Branch:** `x`\n")


def test_execute_workflow_dry_run_stages_tasks(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    prompts_path = repo_root / "plans" / "CURSOR-PROMPTS-DEMO.md"
        ...
        json.dumps(
            {
                "jobs": [
                    {
                        "job_id": "demo",
                        "cursor_prompts_path": "plans/CURSOR-PROMPTS-DEMO.md",
