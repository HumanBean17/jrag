...
Python **3.11+** required, on **Linux, macOS, and Windows**. On Linux, Windows, and **Apple Silicon** Macs every native dependency (LanceDB, LadybugDB, CocoIndex) ships a wheel and you get the full semantic + graph search. **Intel Macs (x86_64) install graph-only**: PyTorch ≥2.3 and LanceDB ≥0.26 dropped macOS Intel wheels, so the vector stack is auto-excluded via PEP 508 markers — `pip install java-codebase-rag` works out of the box, the graph layer (`find` / `describe` / `neighbors` / `resolve`) is fully usable, and the `search` tool falls back to **lexical search** over the symbol graph — BM25-ranked over a LadybugDB full-text index (same tool contract, keyword-ranked instead of semantic; an advisory notes the mode). Semantic/vector search needs Apple Silicon, Linux, or Windows. **Free-threaded (nogil) CPython is not supported**: the free-threaded builds (`python3.13t` / `python3.14t`) cannot run the vector stack because LanceDB ships no free-threaded wheels, and unlike the Intel-Mac case this cannot be auto-excluded — PEP 780's `sys_abi_features` marker is still Draft, so there is no pip-supported way to gate the stack off by interpreter ABI. Use standard CPython (`python3.11`–`python3.14`); on a free-threaded interpreter `pip install` fails resolving `lancedb` (`Cannot install … no matching distribution`). After install, `java-codebase-rag --help` should print the CLI groups.
