...
- Two `neighbors_v2` consumption modes for CALLS: default = resolved-only clean stream; `include_unresolved=True` = interleaved transcript with `row_kind` discriminator. The two modes **compose with `edge_filter`** — the filter applies to resolved rows; unresolved rows pass through unfiltered.
...
| Resolved + candidates found → emit `CALLS(...)` | **Dedup by `(src_id, call_site_line, call_site_byte)` before emit** (Decision 33), then emit `CALLS(..., callee_declaring_role=<parent.role or 'OTHER'>)`. Preference order: (1) concrete-class candidate over interface, (2) same-microservice over cross-microservice, (3) `_lookup_method_candidates` first result. |

`_phantom_method_id` and `tables.phantoms` are deleted in PR-1. Multi-candidate evidence (interface + concrete pointing at the same source-line) is preserved as a build-time `logger.debug` line, not as graph state.
...
**`row_kind` is a discriminator on every `neighbors_v2` edge row, not just CALLS.** All `EdgeRowBase` subclasses gain `row_kind: str = "resolved"`. CALLS resolved rows keep `"resolved"` (renamed from the round-1 amendment's `"call_edge"` for global consistency); CALLS unresolved rows use `"unresolved_call_site"`. Other edge types (HANDLES, OVERRIDES, PUBLISHES_TO, etc.) emit `"resolved"` today; the field is reserved for future edge-type-specific resolution-failure splits without re-shaping the response.

**`include_unresolved` composes with `edge_filter`.** When both are set, `edge_filter` applies to resolved CALLS rows; `UnresolvedCallSite` rows pass through unfiltered (they have no `callee_declaring_role` to filter on — the resolver gave up before reaching a declaring type). This is the workflow for HV38: agent wants "the delegation skeleton **and** the unresolved sites that looked like delegations," because the receiver expression (`orderService.processNext()`) often hints at intent even when the resolver couldn't tie the call to a concrete callee. The asymmetric filter behavior is documented in PR-2's description and locked in Decision 25.
...
| HV28 | `neighbors_v2` with `depth=2` and `edge_filter` | n/a | Filter applied **at every hop**, independently. `neighbors([m], 'out', ['CALLS'], depth=2, edge_filter={callee_declaring_role:'SERVICE'})` returns SERVICE callees of `m`, then SERVICE callees of those. (Locked Decision 21.) |
| HV29 | `find_callers` confidence-filter parity check | `find_callers("save", min_confidence=0.5)` works today | Continues to work. **`find_callers` is not migrated to accept `EdgeFilter`** (Decision 27); the existing discrete `min_confidence` parameter is kept. The asymmetry (`neighbors_v2` uses `EdgeFilter`; `find_callers` uses discrete) is documented in the PR-2 description. |
| HV30 | Telemetry: `GraphMeta` counters | Today: `clients_total`, `producers_total`, `declares_client_total`, `declares_producer_total`, etc. (post SCHEMA-V2) | PR-1 adds three discrete `INT64` counters: `pass3_unresolved_phantom_receiver`, `pass3_unresolved_chained`, `pass3_unresolved_name_only` (one per `reason` value). Decision 23 — column fan-out, not a JSON-encoded map (Kuzu can filter/aggregate these directly without application-layer JSON parsing). |
| HV31 | Interface vs concrete callee declaring role | Today: `pass3_calls` uses `_lookup_method_candidates` which walks supertypes. The declaring-type role on the returned candidate could be the interface (often `OTHER`) or the concrete class (typed). | **Locked Decision 20**: `callee_declaring_role` is sourced from the candidate's `parent_id` Symbol's `role`. `_lookup_method_candidates` already prefers same-microservice and resolved candidates; we don't change that. When resolution lands on an interface declaration (no concrete impl indexed), `callee_declaring_role` reflects the interface's role — which is typically `OTHER` for `JpaRepository`-style interfaces (no stereotype on the interface itself). PR-2 ships a `bank-chat-system` fixture validation that `JpaRepository.save` and `MyRepository extends JpaRepository<...>` both yield `callee_declaring_role='REPOSITORY'` (via the concrete `MyRepository`'s `@Repository` stereotype). The CI test HV31 locks this. |
| HV32 | Brownfield `@CodebaseRole` on declaring type | Today: brownfield role is layered onto the type's `role` via `resolve_role_and_capabilities` (`graph_enrich.py:672`) | `callee_declaring_role` picks it up transparently (it reads `parent.role`, which already reflects the brownfield layer). No additional code. Locked Decision 29 records this. |
| HV33 | `NodeFilter` + `EdgeFilter` composition | n/a | AND across both, ordering preserved. `NodeFilter(microservice='order-service')` + `EdgeFilter(callee_declaring_role='SERVICE')` returns CALLS edges from the queried method to methods whose declaring type role is `SERVICE` *and* whose owning microservice is `order-service`, in `(line, byte)` order. Locked Decision 22. |
| HV34 | The `NodeFilter.role` vs `EdgeFilter.callee_declaring_role` naming-collision trap | n/a | `NodeFilter.role` filters on the **neighbor node's own role**; for a method-kind Symbol that's almost always `OTHER` (inheritance from `_node_row` default). `EdgeFilter.callee_declaring_role` filters on the **callee's declaring type's role**. Both names retained — renaming either is a worse trade. Documented as a callout in `docs/AGENT-GUIDE.md` + a HINTS-V4 hint if `NodeFilter(role=...)` is applied with `edge_types=['CALLS']` and the returned rows are dominantly method-kind symbols with `role='OTHER'` (locked Decision 30). |
| HV35 | Empty-filter performance | n/a | `neighbors([m],'out',['CALLS'])` with no `edge_filter` is the hot path. PR-2 includes a Kuzu predicate-pushdown sanity check: the `callee_declaring_role` column projection on resolved rows must not materially slow the empty-filter case. If profiling shows otherwise, PR-2 adds an index on `(src_id, callee_declaring_role)`. Decision 31 makes this a CI perf invariant (named scenario, not a numeric threshold); the scenario asserts the same `OrderService.process` empty-filter query on `bank-chat-system` returns within 1.5× of its pre-PR-2 median latency on the same hardware. |
| HV36 | `callee_capability` filter request from a future reviewer | n/a | **Out of scope.** Locked Decision 32: `EdgeFilter` exposes only `callee_declaring_role` from the role/capability/annotation triple; capability and annotation filters are *not* added because their cardinality is high and the agent value is unclear. Re-opens via a new propose. |
| HV37 | Multi-candidate resolution at one call site (`JpaRepository.save` interface + `MyRepository.save` concrete) | Today: `pass3_calls` may emit two CALLS rows for the same `(call_site_line, call_site_byte)` with different `dst_id` (interface method id vs concrete method id) and different declaring-type roles. Agent reading source order sees "line 42" twice. | **Locked Decision 33**: `pass3_calls` dedups by `(src_id, call_site_line, call_site_byte)` *before* emitting CALLS rows; preference order is (1) concrete-class candidate over interface, (2) same-microservice over cross-microservice, (3) the first candidate returned by `_lookup_method_candidates` as a tiebreaker. One source-line yields one CALLS edge with one `callee_declaring_role` value. Multi-candidate evidence is preserved as a debuggability log line in the build, not as graph state. PR-1 ships the dedup; PR-1 fixture test on `bank-chat-system` asserts a method calling `myRepository.save(x)` (where `MyRepository extends JpaRepository`) yields one CALLS row with `callee_declaring_role='REPOSITORY'`. |
| HV38 | Delegation skeleton **including** the unresolved sites that looked like delegations | n/a | Agent calls `neighbors([m],'out',['CALLS'], edge_filter={callee_declaring_role:'SERVICE'}, include_unresolved=True)`. Resolved rows are projected to SERVICE-role callees; unresolved rows pass through unfiltered (they have no role). Source order preserved. The agent gets "the four resolved SERVICE delegations + the three sites where the receiver looked like a service but didn't resolve" — enough signal to flag fragile delegation in code review. Locked Decision 25 records the asymmetric filter behavior (resolved rows filtered, unresolved rows unfiltered). |
...
- **HV31** (interface vs. concrete): even with the multi-candidate dedup of HV37/Decision 33, the role projection still depends on `_lookup_method_candidates` returning the concrete impl when one exists. For edge cases where only the interface is indexed (e.g. `JdbcTemplate.query(String, RowMapper<T>, Object...)` with no `@Repository` on `JdbcTemplate`), `callee_declaring_role` will be `OTHER`. Agents will not see those repository hops under a `callee_declaring_role='REPOSITORY'` filter. Mitigation in Decision 20: PR-2 adds an `OTHER`-fallback hint when `callee_declaring_role='SERVICE'`/`'REPOSITORY'` returns 0 results but the unfiltered call has ≥5 results — suggesting the agent try `exclude_callee_declaring_roles=['ENTITY','DTO']` instead.
- **HV35** (perf): "named scenario" rather than "numeric threshold" is the right calibration because Kuzu performance varies by hardware. The test asserts the same empty-filter `OrderService.process` query on `bank-chat-system` is within 1.5× of its pre-PR-2 median latency on the same hardware; if the assertion fails, PR-2 fixes before merge. The pytest scenario id is committed to the PR-2 plan, not this propose.
- **HV38** (compose + unresolved): the asymmetric filter behavior (resolved rows filtered, unresolved rows always emitted) is the smallest workable behavior. Filtering unresolved rows would require an `unresolved_filter` axis on `reason` — a new primitive with no use case behind it. The current asymmetry is documented and self-consistent: `edge_filter` is a *resolved-row* projection knob; `include_unresolved` is an *additional row source*. They compose; they don't interfere.
...
| `callee_microservice` filter axis on `EdgeFilter` | `NodeFilter(microservice=...)` already composes with `EdgeFilter` (HV33 / Decision 22); adding a duplicate axis on the edge would invite confusion about which side owns the filter. Locked out by Decision 32. |
| `EdgeFilter` on `find_callers` / `find_route_callers` | Discrete `min_confidence` parameter is kept; asymmetry with `neighbors_v2` is intentional (HV29 / Decision 27). |
| MCP surface for "find unresolved callers by callee_simple" | Debuggability path lives in `pc unresolved-calls list --callee-simple <name>`; not an agent traversal pattern (Decision 26). |
...
| `unresolved_filter` axis on `reason` (e.g. "include only chained-receiver unresolved sites") | No use case behind it; the workflow for "show me everything" sets `include_unresolved=True` without a reason filter (Decision 25 asymmetry). Re-opens via a new propose if a real workflow emerges. |
...
- Add `include_unresolved: bool = False` to `neighbors_v2`. When `True` and `'CALLS'` in `edge_types`, interleave `UnresolvedCallSite` rows (`row_kind="unresolved_call_site"`) with resolved CALLS rows (`row_kind="resolved"`) in `(call_site_line, call_site_byte)` order. `edge_filter` **composes** with `include_unresolved`: when both are set, the filter applies to resolved rows; unresolved rows pass through unfiltered (Decision 25).
...
- Add `NodeFilter(role=...)` vs `EdgeFilter.callee_declaring_role` collision hint with the refined trigger condition (Decision 30): fires when `NodeFilter(role=...)` is applied to `neighbors([m],'out',['CALLS'])` and the returned rows are dominantly method-kind symbols with `role='OTHER'` (the degenerate-filter case).
...
23. **`GraphMeta` gains three discrete `INT64` counters — one per `reason` value:** `pass3_unresolved_phantom_receiver`, `pass3_unresolved_chained`, `pass3_unresolved_name_only`. Column fan-out, not a JSON-encoded map: Kuzu can `WHERE`/`SUM` these directly without application-layer JSON parsing. Populated in PR-1.
...
25. **`include_unresolved=True` composes with `edge_filter`.** When both are set, `edge_filter` projects resolved CALLS rows; `UnresolvedCallSite` rows pass through unfiltered (they have no `callee_declaring_role`). The asymmetry is intentional: `edge_filter` is a resolved-row projection knob; `include_unresolved` is an additional row source. They don't interfere. HV38 exercises this; HV23 exercises `include_unresolved=True` without `edge_filter`.
26. **No MCP surface for "find unresolved callers by `callee_simple`."** The workflow is debuggability via `pc unresolved-calls list --callee-simple <name>`, not agent traversal.
...
32. **`EdgeFilter` exposes only `callee_declaring_role` from the role/capability/annotation triple.** Capability and annotation filters are out of scope. Re-opens via a new propose if agent value emerges later. `callee_microservice` is also out of scope — use `NodeFilter(microservice=...)` which already composes.
33. **`pass3_calls` dedups multi-candidate resolution by `(src_id, call_site_line, call_site_byte)` *before* emitting CALLS rows.** Preference order: (1) concrete-class candidate over interface, (2) same-microservice over cross-microservice, (3) first candidate returned by `_lookup_method_candidates` as a tiebreaker. One source-line yields one CALLS edge with one `callee_declaring_role` value. Multi-candidate evidence is preserved as a build-time debuggability log line, not as graph state. HV37 locks this in PR-1's `bank-chat-system` fixture test.
...
- **Removed**: HV28 (`cursor-pr-review` use case) and Decision 24 (`cursor-pr-review` as downstream consumer). Rationale: `.cursor` skills in this repo are dev-time tooling, not runtime agent workflows on the indexed codebase. The use-case table is reserved for end-agent / end-user workflows; dev-time skills update independently of the propose's decision log. HV29–HV37 renumbered to HV28–HV36; Decisions 25–33 renumbered to 24–32.
- **Decision 23 reshaped**: `pass3_unresolved_by_reason STRING (JSON map)` replaced with three discrete `INT64` columns (`pass3_unresolved_phantom_receiver`, `pass3_unresolved_chained`, `pass3_unresolved_name_only`). Rationale: JSON-in-a-column is opaque to Kuzu's filter/aggregate; column fan-out is queryable.
- **Decision 25 (was 26) reversed**: `include_unresolved=True` now **composes** with `edge_filter` (filter applies to resolved rows; unresolved rows pass through unfiltered). Rationale: HV38 — the delegation-skeleton-including-unresolved-sites workflow is the most useful interleaved-view query, and the old mutual-exclusivity ruled it out without a use case behind it.
- **Decision 30 (was 31) refined**: collision-hint trigger condition changed from "returns zero rows" to "dominantly method-kind symbols with `role='OTHER'`". Rationale: zero-rows under-fires; dominantly-OTHER is the actual degenerate case.
- **Decision 31 (was 32) refined**: perf scenario now commits to a specific named query (`OrderService.process` empty-filter on `bank-chat-system`) and explicit baseline (same query's pre-PR-2 median latency), with the pytest scenario id deferred to the PR-2 plan. Rationale: HV35 awkward case flagged the hand-waviness.
- **Decision 33 added**: `pass3_calls` dedups multi-candidate resolution by `(src_id, call_site_line, call_site_byte)` *before* emit, with preference order (concrete > interface, same-MS > cross-MS, first candidate as tiebreaker). One source-line, one CALLS edge, one `callee_declaring_role`. New HV37 row covers the test. Rationale: without dedup, an agent reading source order sees the same line twice with conflicting role values.
- **HV38 added**: delegation skeleton + unresolved-but-looks-like-a-service sites; locks the Decision 25 compose behavior.
- **`row_kind` reshaped**: now a global field on `EdgeRowBase` (default `"resolved"`), not CALLS-only. The CALLS family adds `"unresolved_call_site"` for `UnresolvedCallSiteRow`. Rationale: Pydantic discriminated-union shape needs the field on every row type, not just one family.
- **§5 added**: `callee_microservice` filter axis (under Decision 32) and `unresolved_filter` axis (under Decision 25) explicitly out-of-scope. Rationale: pre-empt obvious next-request bolt-ons.
...
- Added Decisions 20–33 to §7 covering interface-vs-concrete resolution, depth>1 semantics, NodeFilter+EdgeFilter composition, GraphMeta telemetry, cross-microservice non-`UnresolvedCallSite` policy, `include_unresolved` semantics, no-unresolved-callers MCP, `find_callers` discrete-parameter asymmetry, package-relativity scope, brownfield role transparency, NodeFilter/EdgeFilter naming collision, perf invariant, no capability/annotation filters.
- Updated TL;DR and PR-2/PR-3 deliverables to match.

**What stayed unchanged from the original draft**:
- Decisions 1–19 unchanged through both revisions.
- Principles 1–8 unchanged through both revisions.
- DDL diff (Appendix A) unchanged through both revisions.
- 3-PR migration sequence unchanged through both revisions.
- HV1–HV20 use-case rows unchanged through both revisions.

**What changed and why** (final state after revision 2):
- Surface B (`include_unresolved`) is a compose-able view, not a mutually-exclusive mode.
- `row_kind` is a global edge-row discriminator, reserved for future resolution-failure splits per edge type.
- `GraphMeta` telemetry is column-fan-out, not JSON-encoded.
- Multi-candidate resolution dedups before emit, giving one CALLS edge per source-line.
- `cursor-pr-review` is *not* in the propose's scope (dev-time tooling, updated alongside PR-1 outside this decision log).
