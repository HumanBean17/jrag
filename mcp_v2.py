...
from mcp_hints import MCP_HINTS_STRUCTURED_FIELD_DESCRIPTION, generate_hints
...
def search_v2(
    ...
    try:
        ...
        if nf and (err := _nodefilter_applicability_error("symbol", nf)):
            ...
            return SearchOutput(success=False, message=err, limit=None, offset=None)
        ...
        raw_struct = generate_hints("search", hint_payload)
        return SearchOutput(
            success=True,
            results=hits,
            limit=limit,
        ...
        return SearchOutput(success=False, message=str(exc), limit=None, offset=None)
...
def find_v2(
    ...
    try:
        ...
        if err := _nodefilter_applicability_error(kind, nf):
            _log_fail_loud("applicability")
            return FindOutput(success=False, message=err, limit=None, offset=None)
        ...
        raw_struct = generate_hints("find", hint_payload)
        return FindOutput(
            success=True,
            results=refs,
            limit=limit,
        ...
        return FindOutput(success=False, message=str(exc), limit=None, offset=None)
...
def describe_v2(
    ...
    try:
        ...
        raw_struct = generate_hints("describe", {"success": True, "record": record.model_dump()})
...
def _resolve_finalize_success(
    ...
    raw_struct = generate_hints("resolve", hint_payload)
...
def neighbors_v2(
    ...
    try:
        ...
        raw_struct = generate_hints("neighbors", neigh_payload)
        return NeighborsOutput(
            success=True,
            results=sliced,
            requested_edge_types=requested_edge_types,
