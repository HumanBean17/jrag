...
You have lexical search and file reading only. Available tools: `Grep` (ripgrep content search), `Glob` (file-path patterns), `Read` (open files), and `Bash`. There is no semantic search and no code-graph index available.
